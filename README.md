...
```sh
npx attestor demo tamper        # 30 seconds: record → verify green → attacker edits → caught → kicker
```
...
✔ ANCHOR   1/1 anchored in Rekor (latest logIndex 2256596856), SET+inclusion verified
...
## Design notes (the parts HN will check first)
...
attestor demo tamper [--offline]    the 30-second pitch
```
...
`packages/toy-mcp-server` (test fixture). `npm test` runs ~60 tests on
